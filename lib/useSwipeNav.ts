...
export function useSwipeNav(opts: {
  ...
  // Avance programmatique (ex: après avoir répondu à une question).
  // `force` permet de contourner `peutAvancer` : celui-ci est calculé à partir
  // du state `reponses` du render précédent (stale closure), donc juste après
  // avoir enregistré une réponse et appelé setReponses(), `peutAvancer` reflète
  // encore l'ancien état et vaudrait `false` à tort. L'appelant (repondre())
  // sait avec certitude, au moment où il appelle avancerAuto(true), qu'une
  // réponse valide vient d'être fournie : l'avancement est donc légitime
  // même si `peutAvancer` n'a pas encore rattrapé le nouveau state.
  const avancerAuto = useCallback((force = false) => {
    if (enTransition || (!peutAvancer && !force)) return;
    setEnTransition(true);
